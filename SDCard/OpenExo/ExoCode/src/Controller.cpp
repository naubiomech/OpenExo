...
float ZeroTorque::calc_motor_cmd()
{
    ...
    if (_controller_data->parameters[controller_defs::zero_torque::use_pid_idx]>0)
    {
    ...
    Serial.print("  PID flag: ");
    Serial.println( _controller_data->parameters[controller_defs::zero_torque::use_pid_idx]);
...
{   //modifying step controller to do a sinosoidal wave 
    
    float Amplitude = _controller_data->parameters[controller_defs::step::amplitude_idx];           //Magnitude of Step Response
    float Duration = _controller_data->parameters[controller_defs::step::duration_idx];             //Duration of Step Response; origninally called Duration, now Frequency
    ...
    */
    ...
    //_controller_data->filtered_torque_reading = utils::ewma(_joint_data->torque_reading, _controller_data->filtered_torque_reading, (_controller_data->parameters[controller_defs::step::alpha_idx])/100);
    _controller_data->filtered_torque_reading = utils::ewma(_joint_data->torque_reading, _controller_data->filtered_torque_reading, 0.5);
    //Serial.println(_controller_data->parameters[controller_defs::step::p_gain_idx]); 
