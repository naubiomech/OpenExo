...
class RtBridge(QtCore.QObject):
    ...
    def feed_bytes(self, data: bytes):
        ...
        # Handshake
        if s == "READY":
            dprint("RtBridge::feed_bytes->Handshake header received")
        ...
        if self._collecting_handshake_payload:
            ...
            if "\n" in self._handshake_payload_buf:
                ...
                dprint("RtBridge::feed_bytes->Handshake payload received:")
                for payload_row in payload_line.split('\n'):
                    if payload_row.strip():
                        dprint(f"    {payload_row}")
                ...
                for block in self._rows_68:
                    dprint(f"RtBridge::feed_bytes->Rows with 68:\n{block}\n")
                for block in self._rows_36:
                    dprint(f"RtBridge::feed_bytes->Rows with 36:\n{block}\n")
                for block in self._rows_38:
                    dprint(f"RtBridge::feed_bytes->Rows with 38:\n{block}\n")
