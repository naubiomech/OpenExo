...
namespace UART_command_handlers
{
    ...
    inline static void get_real_time_data(UARTHandler *handler, ExoData *exo_data, UART_msg_t msg, uint8_t *config)
    {
        ...
        case (uint8_t)config_defs::exo_name::bilateral_hip:
		{
            ...
            rx_msg.data[4] = exo_data->right_side.hip.controller.encoder_angle;
            rx_msg.data[5] = exo_data->right_side.hip.controller.normalized_stance_moment; //hip.controller.encoder_angle;
            rx_msg.data[6] = exo_data->left_side.hip.controller.encoder_angle; //hip.controller.normalized_stance_moment;
