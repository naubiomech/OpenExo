...
, _gait_phase_detector(is_left ? &(exo_data->left_side) : &(exo_data->right_side), exo_data)  //Event-driven bilateral gait phase detector
{
...
void Side::read_data()
{
    ...
    //Update the event-driven gait phase from BOTH feet's FSRs. This only drives
    //the controller for the exo leg; the other side's detector is dormant
    //unless it also wears a hip motor.
    _gait_phase_detector.update();
