...
#include "LogLevels.h"
    ...
    #define Single_Leg_Hip_Board 6 //Single-leg hip exo, only one hip motor, bilateral FSRs for event-driven gait phase
    ...
    /**
     * @brief Configuration for the event-driven gait phase detector
     * (GaitPhaseDetector.h/.cpp).
     * 
     * The detector watches both feet's FSRs. During double support one foot's
     * toe enters its falling edge at (almost) the same time the other foot's
     * heel enters its rising edge. This gives a robust, prediction-free anchor
     * for the start/end of the swing phase of the exo leg.
     */
    namespace gait_phase_config
    {
        const unsigned int CONTACT_DEBOUNCE_MS = 8;     /**< Minimum time a FSR must hold a state before the edge is confirmed. Kills false edges from shuffling/dragging. */
        const unsigned int MIN_SWING_MS = 150;          /**< Min time a swing must last before it is accepted. Filters stomps/hoppping. */
        const unsigned int MAX_SWING_MS = 1500;         /**< Max swing duration. Above this the phase is treated as stalled. */
        const unsigned int SWING_START_LEAD_MS = 40;    /**< How early the flexion torque should start before the detected swing start. Compensates the double-support offset. */
    }
