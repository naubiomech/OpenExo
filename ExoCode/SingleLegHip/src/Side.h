...
class Side
{
	...
	private:
        ...
        //Event-driven gait phase detector. Every side has FSRs (both feet are
        //instrumented), but only the exo leg's detector drives the controller.
        //It reads this side's FSRs AND the contralateral foot's FSRs (via
        //ExoData) so the swing phase is anchored to real FSR edges.
        GaitPhaseDetector _gait_phase_detector; /**< Detects swing/stance from BOTH feet's FSRs. */
