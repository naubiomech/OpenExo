...
{
    //------------------------------------------------------------------
    // Event-driven gait phase (GaitPhaseDetector).
    //
    // The detector anchors the swing phase to real bilateral FSR edges
    // (contralateral heel rising = swing start, exo heel rising / contralateral
    // toe falling = swing end). Here we use its outputs directly. In stance we
    // use ev_percent_stance, in swing we use ev_percent_swing. No expected-
    // duration moving average is involved, so sudden speed changes cannot shift
    // the torque curve relative to the true gait cycle.
    //------------------------------------------------------------------
    bool in_swing = _side_data->in_swing;
    float percent_stance = _side_data->ev_percent_stance;
    float percent_swing = _side_data->ev_percent_swing;
    ...
    float extension_torque_magnitude_Nm = -1 * extension_torque_peak * mass;   /* Sign corrected extension torque magnitude. */
    float flexion_torque_magnitude_Nm = flexion_torque_peak * mass;            /* Sign corrected flexion torque magnitude. */
    ...
    float extension_peak_percent_gait = _controller_data->parameters[controller_defs::franks_collins_hip::trough_percent_gait_idx];         /* % stance where the extension torque curve peaks. */
    float extension_rise_percent_gait = _controller_data->parameters[controller_defs::franks_collins_hip::trough_onset_percent_gait_idx];   /* % stance where the extension torque curve starts. */

    float flexion_peak_percent_gait = _controller_data->parameters[controller_defs::franks_collins_hip::peak_percent_gait_idx];             /* % swing where the flexion torque curve peaks. */
    float flexion_fall_percent_gait = _controller_data->parameters[controller_defs::franks_collins_hip::peak_offset_percent_gait_idx];      /* % swing where the flexion torque curve ends. */

    float extension_fall_percent_gait = (mid_time_percent_gait - (mid_duration_percent_gait / 2)) - extension_peak_percent_gait;            /* % stance where the extension torque curve ends. */
    float flexion_rise_percent_gait = flexion_peak_percent_gait - (mid_time_percent_gait + (mid_duration_percent_gait / 2));                /* % swing where the flexion torque curve starts. */

    //Nodes for the extension spline, keyed on percent stance.
    ...
    //Nodes for the flexion spline, keyed on percent swing.
    float flexion_node1 = -flexion_rise_percent_gait;
    float flexion_node2 = flexion_peak_percent_gait;
    float flexion_node3 = flexion_peak_percent_gait + flexion_fall_percent_gait;
    ...
    if (in_swing)
    {
        ...
        torque_cmd = _spline_generation(flexion_node1, flexion_node2, flexion_node3, flexion_torque_magnitude_Nm, percent_swing);
    }
    else
    {
        //Extension assistance during stance.
        torque_cmd = _spline_generation(extension_node1, extension_node2, extension_node3, extension_torque_magnitude_Nm, percent_stance);
    }
