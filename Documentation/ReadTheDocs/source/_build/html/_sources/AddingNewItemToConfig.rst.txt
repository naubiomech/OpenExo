...
- For each key, add a comment describing its purpose and possible values, for example:
...
- **namespace config_map**  
  ...
  The config file is interpreted as text. This creates a map to convert the text into the ``uint8_t`` declared above.
  An ``IniKeyCode`` type is provided that is then indexed with a ``std::string``. 
...
- For each new key, there are three main steps:
  ...
  1. **Read the key:**  
     Use ``get_section_key(filename, section_name, key_name, buffer, buffer_len)``.
