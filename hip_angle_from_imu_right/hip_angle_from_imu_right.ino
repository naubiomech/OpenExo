...
void setup() {
  ...
  // change the confidence of the observation and process of the kalman filter
  kalman.setQangle(0.01);
  kalman.setQbias(0.01);
  kalman.setRmeasure(0.005);
...
void loop() {
  ...
  if (millis() - prevtime > t) {
    ...
    if (IMU.gyroscopeAvailable() && IMU.accelerationAvailable()) {
      ...
      //float delta_angle = rate * dt + data.estim_angle;
      //data.estim_angle = angle * 0.5 + delta_angle  * 0.5;
    }
    prevtime = millis();
  }
}
...
void sendEvent() {
  ...
  Serial.println("request recieved and data sent");
}
